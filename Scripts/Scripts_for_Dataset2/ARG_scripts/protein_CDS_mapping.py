...
# NOTE: Filenames below correspond to efflux ARG analysis.
# Update filenames accordingly when running inactivation ARG analysis.
# (e.g., replace "efflux" with "inactivation" in input and output filenames).

if __name__ == "__main__":
    extract_top_protein_hits(
        "blastp_unbinned_proteins_against_efflux_CARD_database.xml",
        "Blastp_top_CDS_efflux_hits_from_unbinned_proteins_run1.faa",
        "Blastp_top_CDS_efflux_hits_from_unbinned_proteins_run1.fna",
        "Blastp_efflux_protein_CDS_mapping_dataset2.tsv"
    )
